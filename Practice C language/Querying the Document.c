...
{
    char *answer;
    answer = (char *)calloc(end - beg + 2, sizeof(char));
    int index = 0;
    int i;
    for (i = beg; i <= end; i++)
        answer[index++] = text[i];
...
char **get_sentence(char *text, int beg, int end)
{
    ...
    for (i = beg; i <= end; i++)
        if (text[i] == ' ')
            ++word_count;
    answer = (char **)calloc(word_count, sizeof(char *));
    int start = beg;
    int index = 0;
    for (i = beg; i <= end; i++)
        if (text[i] == ' ')
...
char ***get_paragraph(char *text, int beg, int end)
{
    ...
    for (i = beg; i <= end; i++)
        if (text[i] == '.')
            ++sentence_count;
    answer = (char ***)calloc(sentence_count, sizeof(char **));
    int start = beg;
    int index = 0;
    for (i = beg; i <= end; i++)
        if (text[i] == '.')
...
char ****get_document(char *text)
{
    ...
    for (i = 0; text[i]; i++)
        if (text[i] == '\n')
            ++paragraph_count;
    answer = (char ****)calloc(paragraph_count, sizeof(char ***));
    int start = 0;
    int index = 0;
    for (i = 0; text[i]; i++)
        if (text[i] == '\n')
